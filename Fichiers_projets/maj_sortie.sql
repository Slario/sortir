...
CREATE DEFINER=`root`@`localhost` EVENT `maj de close à en cours` ON SCHEDULE EVERY 1 MINUTE STARTS '2019-09-04 00:00:00' ENDS '2019-10-17 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO UPDATE `sortie` SET `etat`= 'ENC' WHERE (sortie.date_debut < LOCALTIME) AND sortie.etat = 'CLO';
